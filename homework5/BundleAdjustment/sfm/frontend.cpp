...
////////////////////////////////////////////初始化部分///////////////////////////////
    ...
    bool Frontend::Init() {
        ...
        cv::Mat cv_Essential = cv::findEssentialMat(cv_points1, cv_points2, cv_intrinsic_, cv::RANSAC, 0.999, 1.0,
                                                    mask);
        ...
        for (int match_idx = 0; match_idx < matches.size(); match_idx++) {
            ...
            points1.emplace_back(last_frame_->features_[match.queryIdx]->position_.pt.x,
                                              last_frame_->features_[match.queryIdx]->position_.pt.y);
            points2.emplace_back(current_frame_->features_[match.trainIdx]->position_.pt.x,
            ...
            inlier_matches.emplace_back(match.queryIdx, match.trainIdx);
        }
        ...
        // frame.pose中保存的是 从相机到世界的旋转平移(Twc)
        // 分解得到的R: Rc2_c1
        //         T: Tc2_c1
        // ！！！本质矩阵的推理过程中 定义了R T都是从Camera1->Camera2，所以这里得到的R T都是R_c2_c1 T_c2_c1
        // Tc2_c1.inv() == Tc1_c2
        // Pose_wc = Twc1 * Tc1_c2
    ...
    bool Frontend::BuildInitMap() {
        ...
        for (int i = 0; i < n_points; i++) {
            ...
            inliers1.emplace_back(last_frame_->features_[match.first]->position_.pt.x,
                                  last_frame_->features_[match.first]->position_.pt.y);
            inliers2.emplace_back(current_frame_->features_[match.second]->position_.pt.x,
                                  current_frame_->features_[match.second]->position_.pt.y);
        }

        std::vector<Eigen::Vector3d> triangulation_results = TriangulatePoints(proj_mat1,
                                                                               proj_mat2,
                                                                               inliers1,
                                                                               inliers2);
...
////////////////////////////////// Tracking 部分 ////////////////////////////////
    ...
    int Frontend::TrackLastFrame() {
        ...
        // 验证新检测到的点满足几何约束 P2 * K2.T.inv() * E * K.inv() * P1 = 0
