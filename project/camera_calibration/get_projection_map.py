...
class PointSelector(object):
    """
    Refer: https://github.com/neozhaoliang/surround-view-system-introduction/blob/master/surround_view/simple_gui.py#L21
    ---------------------------------------------------
...
if __name__ == "__main__":
    image_path = "/home/sparkai/Pictures/vlcsnap-2023-06-07-16h57m18s312.png"
    image = cv2.imread(image_path, cv2.IMREAD_COLOR)
    factory = 0.5
    n_image = cv2.resize(image, None, None, factory, factory, interpolation=cv2.INTER_LINEAR)
    gui = PointSelector(n_image)
    h, w = image.shape[:2]
    choice = gui.loop()
    if choice > 0:
        src_points = np.float32(gui.keypoints)
        src_points /= factory
        dst_points = np.float32(dst_four_points) * 100
        H = cv2.getPerspectiveTransform(src_points, dst_points)
        print(H)
        H, _ = cv2.findHomography(src_points, dst_points)
        print(H)
        n_image = cv2.warpPerspective(image, H, (1500, 1100))

        cv2.imshow("WarpedImage", n_image)
        cv2.waitKey(0)
